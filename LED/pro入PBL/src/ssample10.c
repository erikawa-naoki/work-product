...
//  int usleep(useconds_t usec); //usecマイクロ秒待機する．usecは1000000（1秒）まで
    ...
    /*for( i=0; i<640; ++i){
     led_canvas_clear(offscreen_canvas); //canvasをクリア
        for(j=0;j<i; ++j){
          led_canvas_set_pixel(offscreen_canvas, (i-j)/10, 31.5*sin(2*p*(i-j)/640)+32, 32*(i/8), 200, 255-4*j); //(i,i)に赤い点を打つ
        }
    ...
    for( m=0; m<1280; ++m){
        for(n=0;n<m; ++n){
          led_canvas_set_pixel(offscreen_canvas, (m-n)/20, 31.5*sin(2*p*(m+n+50)/1280)+32, 32*(m/8), 150, 6*n); //(i,i)に赤い点を打つ
        }
        led_matrix_swap_on_vsync(matrix, offscreen_canvas); //canvasの内容をLEDパネルに転送
